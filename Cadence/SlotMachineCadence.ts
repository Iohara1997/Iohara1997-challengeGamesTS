...
export function slotCadence(symbols: Array<SlotCoordinate>): SlotCadence {
 const { anticipateCadence, defaultCadence, columnSize } = anticipatorConfig
 const slotCadence: number[] = [0];
 let accumulated = 0;

  for (let column = 0; column < columnSize - 1; column++) {
    const countSpecialSymbols = symbols.reduce((acc, curr) => {
      if (curr.column === column) acc++;
      return acc;
    }, 0);
